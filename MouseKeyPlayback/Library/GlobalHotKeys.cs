...
namespace MouseKeyPlayback.Library
{
    ...
    public class GlobalHotKey : IDisposable
    {
        ...
        public static bool RegisterHotKey(ModifierKeys aModifier, Key aKey, Action aAction)
        {
            ...
            if (aRegistered)
            {
                registeredHotKeys.Add(new HotKeyWithAction(aModifier, aKey, aAction,currentID));
            }
            return aRegistered;
        }
        public void Unregister(string aKeyGestureString, Action aAction)
        {
            ...
            var unregister = registeredHotKeys.Where(e => e.Key == aKeyGesture.Key && e.Modifier == aKeyGesture.Modifiers).FirstOrDefault();
