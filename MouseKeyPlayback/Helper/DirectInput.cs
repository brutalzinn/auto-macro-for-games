...
namespace DirectInput
{
    class DirectInput
    {
        ...
        {
            SendInputFlags flags = SendInputFlags.KeyDown | SendInputFlags.UseScanCode;
            short scanCode = GetScanCode(key);
            SendKey(scanCode, (int)flags);
        }

        public static void SendKeyUp(string key)
        {
            SendInputFlags flags = SendInputFlags.KeyUp | SendInputFlags.UseScanCode;
            short scanCode = GetScanCode(key);
            SendKey(scanCode, (int)flags);
        }
        ...
        public static void SendKeyPress(string key, int durationMs = 100)
        {
            SendKeyDown(key);
        ...
        private static short GetScanCode(string key)
        {
            return (short)(int)Enum.Parse(typeof(KeyScanCodes), "DIK_" + key.ToUpper());
        }
